...
!$OMP PARALLEL DEFAULT(PRIVATE) SHARED(r_2_min,force,v_fluid_fluid,n_part,a_type,l_cell,n_cells_tot,n_nei_cells,part_in_cell,lpart_in_cell ,n_cells,r_nei,l_nei,cell_neigh_ls, range_2, r0,inv_boundary, boundary, epsil, sigma_2, e_shift,sig,mass,friction,v, inv_range_2, r_cut_dpd_2)
#ifdef _OPENMP
    ith=omp_get_thread_num()
#endif
...
#if BIN_TYPE == 2 /*cell linked list*/          
         if(i_type .eq. 3) exit ! if particle is solvent, go to next particle. Cell linked list is ordered in such
         !a way that first particles are from brush, and then all the solvent particles come after. 
